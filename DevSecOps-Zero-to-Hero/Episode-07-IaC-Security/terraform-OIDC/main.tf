...
resource "aws_iam_instance_profile" "ec2_profile" {
  name = "${var.environment}-ec2-web-profile"
  role = aws_iam_role.ec2_role.name
}
...
resource "aws_kms_alias" "s3_key_alias" {
  name          = "alias/${var.environment}-s3-app-data-key"
...
resource "aws_sns_topic" "s3_events" {
  name              = "${var.environment}-s3-event-notifications"
...
# S3 Cross-region replication - Replica bucket
resource "aws_s3_bucket" "replica" {
  provider = aws.replica
  bucket   = "${var.environment}-devsecops-app-data-replica"

  tags = {
    Name = "${var.environment}-app-data-replica"
  }
}
...
resource "aws_kms_key" "replica_s3_key" {
  provider                = aws.replica
  ...
  tags = {
    Name = "${var.environment}-replica-s3-kms-key"
  }
}
...
resource "aws_s3_bucket_server_side_encryption_configuration" "replica" {
  ...
  rule {
    apply_server_side_encryption_by_default {
      sse_algorithm     = "aws:kms"
      kms_master_key_id = aws_kms_key.replica_s3_key.arn
    }
    bucket_key_enabled = true
  }
}
...
resource "aws_sns_topic" "replica_s3_events" {
  provider          = aws.replica
  name              = "${var.environment}-replica-s3-event-notifications"
  kms_master_key_id = aws_kms_key.replica_s3_key.id

  tags = {
    Name = "${var.environment}-replica-s3-events"
  }
}

resource "aws_sns_topic_policy" "replica_s3_events" {
  provider = aws.replica
  arn      = aws_sns_topic.replica_s3_events.arn
    ...
    Statement = [
      {
        ...
        Action   = "SNS:Publish"
        Resource = aws_sns_topic.replica_s3_events.arn
        Condition = {
          ArnLike = {
...
resource "aws_s3_bucket_notification" "replica" {
  ...
  topic {
    topic_arn = aws_sns_topic.replica_s3_events.arn
    events    = ["s3:ObjectCreated:*", "s3:ObjectRemoved:*"]
  }
}
